...
Функция ЗаполнитьТекстПроцедурыПКО(МакетТекстовМодулей, ТекстПроцедурыВызоваОбработчика)
	...
	|	ТабПКС_ТЧ.НомерСтроки КАК НомерСтроки
	|ИЗ ТабПКС_ТЧ
	|УПОРЯДОЧИТЬ ПО
	|	ТабПКС_ТЧ.СвойствоКонфигурации,
	|	ТабПКС_ТЧ.СвойствоФормата,
	|	ТабПКС_ТЧ.НомерСтроки
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ЗаполнитьТекстПроцедурыАлгоритмы(МакетТекстовМодулей, ТекстПроцедурыВызоваОбработчика)
	...
	ТекстКоммент = СтрЗаменить(ОбластьКомментарий.ПолучитьТекст(), "#Область Конвертация_Алгоритмы_Комментарий","");
	ТекстПроцедуры.ДобавитьСтроку(ТекстКоммент);
